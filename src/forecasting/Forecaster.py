...
def run_forecasts():
	...
	home_away_map = MyConstants.DEFAULT_HOME_AWAY_MAP
	...
	for player in players:
		...
		es_results = exponential_smoothing(actual_keys, forecast_keys, MyConstants.SMOOTHING_FACTOR, player, oppositions, team_map, home_away, home_away_map)
		wa_results = weighted_average(actual_keys, forecast_keys, player, oppositions, team_map, home_away, home_away_map)
...
def last_game(actual_gws, forecast_gws, player, oppositions, team_map, home_away, home_away_map):
	...
	for idx, key in enumerate(actual_gws):
		...
		for fixture in range(0, len(oppositions[idx-1])):
			forecast += (points[-1] * team_map[oppositions[idx-1][fixture]] * home_away_map[home_away[idx-1][fixture]])
...
def weighted_average(actual_gws, forecast_gws, player, oppositions, team_map, home_away, home_away_map):
	...
	for idx, key in enumerate(actual_gws):
		...
		forecast = 0
		for fixture in range(0, len(oppositions[idx-1])):
			forecast += (mean([last_game,last_3_mean,last_6_mean]) * team_map[oppositions[idx-1][fixture]] * home_away_map[home_away[idx-1][fixture]])
		forecasts[forecast_gws[idx - 1]] = forecast
	return forecasts
...
def exponential_smoothing(actual_gws, forecast_gws, smoothing_factor, player, oppositions, team_map, home_away, home_away_map):
	...
	for idx, key in enumerate(actual_gws):
		...
		for fixture in range(0,len(oppositions[idx])):
			if idx == 0:
				nforecast = player["fixture_history"][key]["points"] * team_map[oppositions[idx][fixture]] * home_away_map[home_away[idx][fixture]]
			else:
			 	nforecast = forecasts[forecast_gws[idx - 1]] + (smoothing_factor * (player['fixture_history'][forecast_gws[idx-1]]['points'] - forecasts[forecast_gws[idx -1]]))
...
def get_maps(players):
	...
	for player in players:
		fh = player["fixture_history"]
		for fixture in fh:
			opponent = fh[fixture]["opponent_result"][:3]
			if opponent not in team_map:
				team_map[opponent] = 0
			team_map[opponent] += fh[fixture]["points"]

			home_away = fh[fixture]["opponent_result"][4:5]
			home_away_map[home_away] += fh[fixture]["points"]
