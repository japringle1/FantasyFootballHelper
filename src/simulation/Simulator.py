...
from squadselection import Substitutions
...
def run_simulation(forecast_type):
	...
	for gw in range(startGW, MyConstants.GW_COUNT+1):
		...
		# Get squads
		results = SquadSelection.get_combinations(MyConstants.FORECASTS_COLLECTION, gw, budget, forecast_type)
	...
	if (MyConstants.GW_COUNT+1 < 39):
		...
		results = SquadSelection.get_combinations(MyConstants.FORECASTS_COLLECTION, MyConstants.GW_COUNT+1, 1000, forecast_type)
		team = results[-1]
